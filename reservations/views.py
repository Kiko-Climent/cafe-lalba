    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if booking.date and booking.start_time and booking.end_time:
                ...
                colliding_reservations = Booking.objects.filter(
                    date=booking.date
                ).exclude(
                ...
                if colliding_reservations.count() >= 6:
                    last_booking_time = colliding_reservations.latest(
                        'date_added').date_added
                    if last_booking_time + timedelta(hours=2) > timezone.now():
                        messages.error(request, 'Fully booked for this hour.'
                                       f' Try another time.')

                        return render(
                            request, 'reservations/new_reservation.html',
                            {'form': form})
            ...
            report_html = render_to_string(
                    'reservations/reservation_report.html', {
            ...
            messages.error(
                    request, 'Something went wrong, please check your data')
    else:
        form = BookingForm()
...
def user_reservation(request):
    ...
    if user_bookings.exists():
        ...
        if active_booking.user == request.user:
            print(
                "User and active booking associated correctly:",
                request.user, active_booking.id)
            return render(
                request, 'reservations/update_reservation.html', {
                    'form': BookingForm(instance=active_booking),
...
def reservation_report(request, booking_id):
    ...
    if booking:
        ...
        return render(
            request, 'error.html',
            {'message': 'Booking not found or not associated with the user'})
...
def reservation_list(request):
    ...
    return render(
         request, 'reservations/reservation_list.html', {'bookings': bookings})
...
def delete_reservation(request, booking_id):
    ...
    return render(
         request, 'reservations/delete_reservation.html', {'booking': booking})
